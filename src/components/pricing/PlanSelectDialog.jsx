...
export default function PlanSelectDialog({
  ...
  const handleUpgrade = async (plan, billingCycle) => {
    ...
    try {
      ...
      alert(`Error: ${error.message || "An unexpected error occurred during checkout"}`);
    } finally {
      setLoadingPlan(null);
    }
  };

  // Handle success redirect and refresh user data
  useEffect(() => {
    const handleSuccessRedirect = async () => {
      if (hasProcessed.current) return; // Prevent re-processing
      const query = new URLSearchParams(location.search);
      const sessionId = query.get("session_id");
      if (sessionId && location.pathname.includes("/payment/success")) {
        hasProcessed.current = true;
        try {
          console.log("Processing success redirect with sessionId:", sessionId);
          // Call backend success endpoint
          const successResponse = await api.get(`/api/v1/stripe/success?session_id=${sessionId}`, {
            withCredentials: true,
          });
          console.log("Success endpoint response:", successResponse.data);
          // Refresh user data
          const featuresResponse = await api.get("/api/v1/features", { withCredentials: true });
          dispatch(updateUserSuccess(featuresResponse.data.user));
          console.log("User data refreshed:", featuresResponse.data.user);
          navigate("/payment/success", { replace: true }); // Clear query params
          onClose(); // Close dialog
        } catch (error) {
          console.error("Success redirect error:", {
            message: error.message,
            response: error.response?.data,
    ...
    return () => {
      hasProcessed.current = false; // Reset on unmount
    };
  }, [location, dispatch, navigate, onClose]);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      >
                        ...
                        <div className="flex justify-center">
                          ...
                          >
                            {loadingPlan === `${selectedPlan}-12_months` ? "Processing..." : "Buy Now"}
                          </button>
                        </div>
                      </div>
                      <div
                        ...
                        style={{
                          ...
                          borderImageSource:
                            "linear-gradient(180deg, rgba(136, 39, 118, 0.4) 17.19%, rgba(251, 98, 228, 0.77) 100%), linear-gradient(0deg, rgba(255, 255, 255, 0.2)), linear-gradient(180deg, rgba(136, 39, 118, 0) -4.69%, rgba(254, 189, 245, 0.3))",
                          borderImageSlice: 1,
                          backdropFilter: "blur(17.94871711730957px)",
                          boxShadow:
                          ...
                          <button
                            ...
                            style={{
                              background:
                                "linear-gradient(180deg, rgba(0, 0, 0, 0) -40.91%, #882776 132.95%)",
                              borderImageSource:
                                "linear-gradient(180deg, rgba(136, 39, 118, 0.4) 17.19%, rgba(251, 98, 241, 0.77) 100%), linear-gradient(0deg, rgba(255, 255, 255, 0.2)), linear-gradient(180deg, rgba(136, 39, 118, 0) -4.69%, rgba(254, 189, 253, 0.3))",
                              borderImageSlice: "1",
                              gap: "10.77px",
                              backdropFilter: "blur(17.94871711730957px)",
                              boxShadow:
                                "0px 8.97px 26.92px 0px rgba(255, 73, 243, 0.7) inset, 0px 8.97px 35.9px 0px rgba(175, 63, 160, 0.5)",
                            }}
                          >
                            {loadingPlan === `${selectedPlan}-3_months` ? "Processing..." : "Buy Now"}
                          </button>
                        </div>
                      </div>
                      <div
                        ...
                        style={{
                          ...
                          borderImageSource:
                            "linear-gradient(180deg, rgba(39, 67, 136, 0.4) 17.19%, rgba(98, 140, 251, 0.77) 100%), linear-gradient(0deg, rgba(255, 255, 255, 0.2)), linear-gradient(180deg, rgba(39, 59, 136, 0) -4.69%, rgba(189, 200, 254, 0.3))",
                          borderImageSlice: 1,
                          backdropFilter: "blur(17.94871711730957px)",
                          boxShadow:
                            "0px 8.97px 26.92px 0px rgba(73, 96, 255, 0.7) inset, 0px 8.97px 35.9px 0px rgba(63, 81, 175, 0.5)",
                        }}
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div>
                        ...
                        <div className="flex justify-center">
                          <button
                            onClick={() => {
                              console.log(`Selected ${selectedPlan.toUpperCase()} Quarterly Plan`);
                              handleUpgrade(selectedPlan, "1_month");
                            }}
                            className={`w-[165px] h-[35px] rounded-[7.18px] font-semibold text-white transition-all duration-300 ${
                              loadingPlan === `${selectedPlan}-1_month` ? "opacity-50 cursor-not-allowed" : ""
                            }`}
                            ...
                            style={{
                              background:
                                "linear-gradient(180deg, rgba(0, 0, 0, 0) -40.91%, #200B5C 132.95%)",
                              borderImageSource:
                                "linear-gradient(180deg, rgba(11, 43, 92, 0.4) 17.19%, rgba(101, 98, 251, 0.77) 100%), linear-gradient(0deg, rgba(255, 255, 255, 0.2)), linear-gradient(180deg, rgba(11, 48, 92, 0) -4.69%, rgba(189, 211, 254, 0.3))",
                              borderImageSlice: "1",
                              gap: "10.77px",
                              backdropFilter: "blur(17.94871711730957px)",
                              boxShadow:
                                "0px 8.97px 26.92px 0px rgba(109, 73, 255, 0.7) inset, 0px 8.97px 35.9px 0px rgba(32, 11, 92, 0.5)",
                            }}
                          >
                            {loadingPlan === `${selectedPlan}-1_month` ? "Processing..." : "Buy Now"}
                          </button>
                        </div>
                      </div>
                    </div>
